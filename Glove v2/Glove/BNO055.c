...
void BNO_Init(void)
{
	...
	for (int i = 0; i < MAX_IMU_COUNT; ++i)
	{
		...
		i2c_write(BNO055_RESET);			//Set operation mode to IMU (Reset und nicht IMU mode? TODO: nachschauen)
		i2c_stop();
...
void BNO_Read_Calibration_Data(uint8_t id, uint8_t* buffer_calibration_data) {
		if (!BNO_is_available(id))
		{
			return;
		}
		...
		i2c_start_wait(BNO055_ADDRESS + I2C_WRITE);			//Set device address and write mode
		i2c_write(BNO055_CALIB_STAT_ADDR);					//Access CALIB_STAT data
		i2c_rep_start(BNO055_ADDRESS + I2C_READ);			//Set device address and read mode
		buffer_calibration_data[0] = i2c_readNak();		//Read CALIB_STAT BNOx
		i2c_stop();
}
